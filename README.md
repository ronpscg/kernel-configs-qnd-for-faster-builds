...
Running example:
The following example runs hvc0 as the main console in stdio as well with the QEMU monitor , and also spawns a console in tty0 (the graphical one), and also adds a network device:
```
qemu-system-x86_64 -kernel /home/ron/dev/linux-6.15.0-out/x86_64/arch/x86/boot/bzImage -enable-kvm  \
    ...
    -append "stopatramdisk earlycon=hvc0 console=tty0 console=hvc0 net.ifnames=0 pscgrd.hw.bsp=qemu" -m 2048  \
    -netdev user,id=net0 -device virtio-net-pci,netdev=net0 \
    -chardev stdio,id=virtiocon0,mux=on,signal=off  -device virtio-serial-pci   -device virtconsole,chardev=virtiocon0 -mon chardev=virtiocon0,mode=readline
```
...
On x86_64 the HVC is really unnecessary, as **it requires PCI anyhow**, but building it like this is still super fast, and it allows to better unify the command lines of other builds,
other than the annoying *virtio-serial-pci* obligation which is not necessasry on other platforms, which use *virtio-serial-device* to avoid needing `CONFIG_PCI`

**Main Problem here** - earlycon / earlyprintk do not go to HVC and neither is anything before it is being initialized, so it's better to use /dev/ttyS0 or /dev/tty0 if using FRAMEBUFFER_CONSOLE
...
Or to use tty0 on the "display", and hvc as the main console:
```
...
Or to use ttyS0 on stdio, and hvc as the secondary console (without display)
```
qemu-system-x86_64 -kernel ./arch/x86/boot/bzImage -enable-kvm -initrd /home/ron/pscgbuildos-builds/target/product/pscg_debos/build-x86_64/image_materials_workdir/installables/bootfat/initramfs.cpio -append "stopatramdisk  earlyprintk=hvc0 console=hvc0 console=ttyS0 net.ifnames=0 pscgrd.hw.bsp=qemu" -m 2048   -serial mon:stdio -netdev user,id=net0 -device virtio-net-pci,netdev=net0 -chardev socket,id=hvc0_socket,path=/tmp/qemu_hvc0.sock,server=on,wait=on    -device virtio-serial-pci   -device virtconsole,chardev=hvc0_socket -display none
```
...
**So to wrap it up and summarize, for example, if you want to have**
